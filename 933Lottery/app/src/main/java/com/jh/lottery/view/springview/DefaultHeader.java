package com.jh.lottery.view.springview;
...
public class DefaultHeader extends BaseHeader {
    ...
    public DefaultHeader(Context context){
        this(context, R.drawable.progress_small, R.mipmap.arrow_pull);
    }
    ...
    public void onPreDrag(View rootView) {
        if (freshTime==0){
            ...
            if(m>=1 && m<60){
                headerTime.setText( m +context.getString(R.string.ali_just_m));
            }else if (m>=60){
                int h = m/60;
                headerTime.setText( h +context.getString(R.string.ali_just_h));
            }else if(m>60*24){
                int d = m/(60*24);
                headerTime.setText( d +context.getString(R.string.ali_just_d));
            }else if(m==0){
                headerTime.setText(context.getString(R.string.ali_just_now));
            }
        }
    }
    ...
    public void onLimitDes(View rootView, boolean upORdown) {
        if (!upORdown){
            headerTitle.setText(context.getString(R.string.ali_header_text_release));
            if (headerArrow.getVisibility()== View.VISIBLE)
                headerArrow.startAnimation(mRotateUpAnim);
        }
        else {
            headerTitle.setText(context.getString(R.string.ali_header_text));
            if (headerArrow.getVisibility()== View.VISIBLE)
    ...
    public void onStartAnim() {
        freshTime = System.currentTimeMillis();
        headerTitle.setText(context.getString(R.string.ali_header_text_ing));
